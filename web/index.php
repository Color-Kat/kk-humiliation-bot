...
function checkObjInArr($field, $arr)
{
...
$app->post('/bot', function () use ($app) {
    ...
    switch ($data->type) {
        ...
        case 'message_new':
            ...
            // show error
            // file_put_contents('logs.txt', file_get_contents('https://api.vk.com/method/messages.send?' . http_build_query($request_params)));
            ...
            $insults = [
                ...
                'хряк',
                'осиротел',
                'щетинестая феменистка',
                'участник украинского гей парада',
                'гнида',
                'седьмая ведьма винкс',
                'бесплоден',
                'холодец',
                'холодец застывший',
                'кишкоблуд',
            ...
            if (
                ...
                if (rand(0, 20) == 20) {
                    $yes = array_merge($yes, [
                        'молодец, ты наконец-то понял!',
                        'умница, я смог это вразумить!',
                        'красаучик, дошло в коем-то веке',
                        'до него дошло, что он '. $insults[$random_insult_number] . '!!!!',
                        'урааа',
                        'святой Густов мной очень доволен',
                        'супер, я исправна выполняю свой долг!',
                        'замечательно, ты '. $insults[$random_insult_number],
                        'о, круто, а я уже хотел было на тебя ругаться',
                        'признался)',
                        'слит)'
                    ]);
                }
            ...
            if (
                ...
                $fwd_message = [
                    ...
                    'в жопу себе это перешли',
                    '"цитаты великих людей"',
                    'это не моё!',
                    'зачем ты мне это переслал? я итак понимаю',
                    'я этого не писал, ты ничего не докажешь',
                    'не, не, это не моё, я чист и не винен',
                    'это чьё? эт не моё, я не виноват',
                    'ну, это же статья',
                    'и чё? в жопу перешли это сообщние',
                    'в жопу себе сообщение перешли',
                ];
            ...
            // === пользователь отправил запись === //
            if (
                // count($data->object->attachments) ||
                checkObjInArr('type', $data->object->attachments) == 'photo' ||
                checkObjInArr('type', $data->object->attachments) == 'wall'
            ) {
                $attachments = [
                    'че это за хрень?',
                    'у тебя плохое чувство юмора, ' . $insults[$random_insult_number],
                    'баян',
                    'я видел уже, ' . $insults[$random_insult_number],
                    'какой-то ты отсталый, я уже 10 раз видел эту лажу',
                    'эт шляпа какая-то',
                    'задолбёшь хрень мне всякую кидать',
                    'это лажа полная вообще',
                    'ХАПХПАХПЫХАХВАХЫ',
                    'ору, баян',
                    'апхап, ты посмотри, что творится',
                    'смешно -_-',
                    'баянище',
                    'сто раз уже видел',
                ];

                $request_params['message'] = $attachments[array_rand($attachments)];
            }
